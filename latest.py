...
# Global registry for patterns
pattern_registry = {}
pattern_counter = 1


def register_pattern(pattern):
    """Store unique regex pattern and return its assigned name"""
    global pattern_counter
    if pattern not in pattern_registry:
        key = f"text_{pattern_counter}"
        pattern_registry[pattern] = key
        pattern_counter += 1
...
def parse_restrictions(elem):
    ...
    if restriction is not None:
        ...
        for p in restriction.findall("xs:pattern", ns):
            ...
            if pat_value:
                key = register_pattern(pat_value)
                restrictions.setdefault("patterns", []).append(key)
...
if __name__ == "__main__":
    ...
    # Reverse mapping: text_X -> regex
    reversed_patterns = {v: k for k, v in pattern_registry.items()}

    # Save patterns.json
    with open("patterns.json", "w") as f:
        json.dump(reversed_patterns, f, indent=4)
